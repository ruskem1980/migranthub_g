...
export default registerAs('passportValidity', () => ({
  ...
  serviceUrl: process.env.PASSPORT_VALIDITY_SERVICE_URL || 'https://services.fms.gov.ru/info-service.htm?sid=2000',
  ...
  circuitBreakerThreshold: parseInt(process.env.PASSPORT_VALIDITY_CIRCUIT_BREAKER_THRESHOLD || '5', 10),

  // Circuit breaker: время восстановления в миллисекундах
  circuitBreakerResetTime: parseInt(process.env.PASSPORT_VALIDITY_CIRCUIT_BREAKER_RESET_TIME || '60000', 10),

  // TTL кэша результатов в миллисекундах (7 дней)
  cacheTtl: parseInt(process.env.PASSPORT_VALIDITY_CACHE_TTL || String(7 * 24 * 60 * 60 * 1000), 10),
