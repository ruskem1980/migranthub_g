 ...
 * Searches in questions, answers and tags
 */
export function searchKnowledge(
  ...
  return KNOWLEDGE_BASE.filter((item) => {
    ...
    const aMatches = keywords.filter((k) =>
      [a.question[lang], a.answer[lang], ...a.tags]
        .join(' ')
        .toLowerCase()
        .includes(k)
    ).length;
    ...
    const bMatches = keywords.filter((k) =>
      [b.question[lang], b.answer[lang], ...b.tags]
        .join(' ')
        .toLowerCase()
        .includes(k)
    ).length;
 ...
 */
export function getKnowledgeCategories(): Record<KnowledgeCategory, number> {
...
/**
 * Get random knowledge items (useful for suggestions)
 */
export function getRandomKnowledge(count: number = 3): KnowledgeItem[] {
