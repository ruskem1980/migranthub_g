...
test.describe('Тренажер - начальный экран', () => {
  // Тест временно отключен - требует рефакторинга после изменения UI
  test.skip('Показывает начальный экран с выбором категории', async ({ page }) => {
    console.log('\n🧪 Тест начального экрана тренажера\n');
  ...
  // Тест временно отключен - требует рефакторинга после изменения UI
  test.skip('Выбор категории "Русский язык"', async ({ page }) => {
